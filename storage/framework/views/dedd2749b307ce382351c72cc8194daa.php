<div class="mb-3">
    <div class="card shadow-none b-r-6">
        <div class="card-header px-3">
            <div class="fs-12 fw-6 text-gray-700">
                <?php echo e(__("YouTube")); ?>

            </div>
        </div>
        <div class="card-body px-3">
            <?php
                $ytDisclaimer = isset($postData) && isset($postData->options)
                    ? ($postData->options->yt_disclaimer ?? '')
                    : '';
            ?>
            <div class="mb-3">
                <label class="form-label"><?php echo e(__('Disclaimer')); ?></label>
                <textarea class="form-control input-emoji bbr-r-6 bbl-r-6" name="options[yt_disclaimer]"><?php echo e($ytDisclaimer); ?></textarea>
            </div>
            <div class="mb-0">
                <label class="form-label"><?php echo e(__('Post To')); ?></label>
                <div class="d-flex gap-8 flex-column flex-lg-row flex-md-column">
                    <div class="form-check me-3">
                        <input class="form-check-input" type="radio" name="options[yt_type]" value="video" id="youtube_type_video" checked>
                        <label class="form-check-label mt-1" for="youtube_type_video">
                            <?php echo e(__('YouTube Video')); ?>

                        </label>
                    </div>
                    <div class="form-check me-3">
                        <input class="form-check-input" type="radio" name="options[yt_type]" value="shorts" id="youtube_type_shorts">
                        <label class="form-check-label mt-1" for="youtube_type_shorts">
                            <?php echo e(__('YouTube Shorts')); ?>

                        </label>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<?php /**PATH /var/www/social.kokonuts.my/modules/AppChannelYoutubeProfiles/resources/views/options.blade.php ENDPATH**/ ?>